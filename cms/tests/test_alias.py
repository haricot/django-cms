...
class AliasTestCase(TransactionCMSTestCase):
    ...
    def test_move_and_delete_plugin_alias(self):
        ...
        ph_en = page_en.placeholders.get(slot="col_left")    def test_alias_recursion_across_page_translation(self):
        page = api.create_page(
            "Alias plugin",
            "col_two.html",
            "en",
        ...
        api.add_plugin(ph_1, 'StylePlugin', 'en', tag_type='div', class_name='info_1')
        source_plugin_ph2 = api.add_plugin(ph_2, 'StylePlugin', 'en', tag_type='div', class_name='info_2')

        api.create_title("fr", "page_1", page)
        page.publish('fr')

        alias_placeholder = api.add_plugin(ph_1, 'AliasPlugin', 'fr', alias_placeholder=ph_1)
        alias_plugin = api.add_plugin(ph_2, 'AliasPlugin', 'fr', plugin=source_plugin_ph2)
